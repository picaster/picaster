...
{
    std::cerr << "JACK shutdown, exiting." << std::endl;
    exit(1);
}
...
JackClient::stopJack()
{
    std::cerr << "enter stopJack" << std::endl;
    if (client == NULL) return;
    jack_client_close(client);
    client = NULL;
    if (!detached)
    {
        std::cerr << "stopJack : killing jackd (" << this->server_pid << ")" << std::endl;
        kill(this->server_pid, SIGKILL);
    }
    std::cerr << "exit stopJack" << std::endl;
}
...
void
JackClient::startJack(void (*callback)())
{
    if (client != NULL) return;
