...
JackFilePlayerModule*
on_fx_play(GtkButton* button, const char* file_path)
{
    if (context.fx->isPlaying()) return NULL;
    context.fx->load(file_path);
    context.fx->play();
    /*gint callback_id = */g_timeout_add(500, (GSourceFunc)on_track_update, button);
    return context.fx;
}
...
extern "C" {
    ...
    {
        on_button_clicked(button, on_fx_play, on_track_stop, user_data);
    }
}
